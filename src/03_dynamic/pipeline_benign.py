...
MAX_SECONDS_PER_APK = ANALYSIS_WAIT + 120
...
def output_path(year: int) -> Path:
    return Path(f"data/features/dynamic_features_benign/{year}/dynamic_features_5min_v2.parquet")
...
def build_cmd(year: int, serials: list[str]) -> list[str]:
    cmd = [
        sys.executable, "src/03_dynamic/pipeline_parallel.py",
...
def subprocess_timeout(total_apks: int, done: int, n_emus: int) -> int:
    ...
    per_emu   = (remaining + n_emus - 1) // n_emus
