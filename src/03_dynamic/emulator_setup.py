...
def sdk_tool(name: str) -> Path | None:
    ...
    if ct.exists():
        for sub in sorted(ct.iterdir(), reverse=True):
...
def run_pipeline(years: list[int], avds: list[str] | None = None, malware: bool = False):
    script    = "src/03_dynamic/pipeline_malware.py" if malware else "src/03_dynamic/pipeline_benign.py"
...
def main():
    ...
    parser.add_argument(
        ...
        help="Malware analizi yap (pipeline_malware.py)"
    )
